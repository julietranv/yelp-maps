...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    return [mean([restaurant_location(r)[i] for r in cluster]) for i in range(2)]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    s_xx = sum([r**2 for r in [r - mean(xs) for r in xs]])
    s_yy = sum([r**2 for r in [r - mean(ys) for r in ys]])

    zipped = zip([r - mean(xs) for r in xs], [r - mean(ys) for r in ys])
    s_xy = sum([r[0] * r[1] for r in zipped])
